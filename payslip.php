...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['check_in'])) {
        ...
        if ($result->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO emp_attendance (empcode, date, check_in_time, check_out_time, work_hours, status) 
                                    VALUES (?, ?, ?, ?, ?, 'present')");
            $stmt->bind_param("sssss", $empcode, $current_date, $check_in_time, $check_out_time, $work_hours);
            $stmt->execute();
...
<body>
    ...
    <div class="card">
        ...
        <form method="post" action="payslip.php" class="form-container">
    ...
    <table class="table table-hover table-striped table-checkable order-column full-width">
        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                ...
                echo "<tr><td colspan='7' style='text-align: center;'>No attendance records for today</td></tr>";
            }
            // Get today's date
            $current_date = date('Y-m-d');
